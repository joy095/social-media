...
class SocialController {
  ...
  // @access  Private
  async getPostLikes(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const post = await Post.findById(req.params.id)
  ...
  // @access  Private
  async getUserFollowers(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const page = parseInt(req.query.page as string) || 1;
      const limit = parseInt(req.query.limit as string) || 20;
      const skip = (page - 1) * limit;
  ...
  // @access  Private
  async getUserFollowing(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const page = parseInt(req.query.page as string) || 1;
      const limit = parseInt(req.query.limit as string) || 20;
      const skip = (page - 1) * limit;
  ...
  // @access  Private
  async getFollowSuggestions(req: AuthRequest, res: Response, next: NextFunction) {
    try {
      const currentUser = await User.findById(req.user.id);
  ...
  private async canUserInteractWithPost(post: any, userId: string): Promise<boolean> {
  ...
  private detectBotBehavior(user: any, action: 'like' | 'view'): boolean {
    const now = new Date();
