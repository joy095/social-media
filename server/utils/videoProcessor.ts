...
export const processVideo = (inputPath: string): Promise<ProcessedVideo> => {
  ...
  return new Promise((resolve, reject) => {
    const outputFileName = outputPath || 
      path.join(
        ...
        `${path.basename(videoPath, path.extname(videoPath))}_thumb_${Date.now()}.jpg`
      );
  ...
  return new Promise((resolve, reject) => {
    const output = outputPath || 
      path.join(
        ...
        `${path.basename(inputPath, path.extname(inputPath))}_compressed_${Date.now()}.mp4`
      );
...
export const convertVideoFormat = (
  inputPath: string,
  outputFormat: string,
  outputPath?: string
): Promise<string> => {
  return new Promise((resolve, reject) => {
    const output = outputPath || 
      path.join(
        path.dirname(inputPath),
        `${path.basename(inputPath, path.extname(inputPath))}_converted_${Date.now()}.${outputFormat}`
      );
...
export const cleanupVideoFiles = (filePaths: string[]): void => {
