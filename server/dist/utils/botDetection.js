...
class BotDetector {
    ...
    recordInteraction(userId, actionType, ip, userAgent) {
        ...
        userData.interactions = userData.interactions.filter(interaction => now - interaction.timestamp < 10 * 60 * 1000 // 10 minutes
        );
    ...
    analyzeBotBehavior(userKey, userData) {
        ...
        const recentInteractions = interactions.filter(i => now - i.timestamp < BOT_PATTERNS.RAPID_INTERACTIONS.timeWindow);
        ...
        // Check user agent
        if (userAgent && BOT_PATTERNS.SUSPICIOUS_USER_AGENTS.some(pattern => userAgent.toLowerCase().includes(pattern))) {
            suspiciousScore += 40;
