...
const validateR2Config = () => {
    ...
    if (!accountId || !accessKeyId || !secretAccessKey || !bucketName || !publicUrl) {
        ...
        if (!accessKeyId)
            console.error("- CLOUDFLARE_R2_ACCESS_KEY_ID is missing");
        if (!secretAccessKey)
            console.error("- CLOUDFLARE_R2_SECRET_ACCESS_KEY is missing");
        if (!bucketName)
            console.error("- CLOUDFLARE_R2_BUCKET_NAME is missing");
        if (!publicUrl)
            console.error("- CLOUDFLARE_R2_PUBLIC_URL is missing");
        return false;
    }
    return true;
};
...
const ensureError = (error) => {
    ...
    if (typeof error === 'object' && error !== null) {
        ...
        properError.http_code = httpCode;
        properError.originalObject = error;
        return properError;
    }
    return new Error(String(error));
};
// Generate unique key for R2
const generateKey = (filePath, folder = "social-media-upload") => {
...
// Upload file to R2
export const uploadToR2 = async (filePath, folder = "social-media-upload") => {
    if (!isR2Configured) {
    ...
    try {
        ...
        console.error("✗ R2 upload failed:", {
            ...
            http_code: properError.http_code,
        ...
        const httpCode = properError.http_code || properError.statusCode;
        throw new R2UploadError(`R2 upload failed: ${errorMessage}`, httpCode);
    }
};
// Upload video to R2 (alias for uploadToR2)
export const uploadVideoToR2 = async (filePath, folder = "social-media-upload") => {
...
export const getVideoThumbnail = (key, options = {}) => {
    // For R2, you'd need to generate thumbnails separately
    // This is a placeholder that returns the video URL
    return `${process.env.CLOUDFLARE_R2_PUBLIC_URL}/${key}`;
};
// Upload multiple files to R2
export const uploadMultipleToR2 = async (filePaths, folder = "social-media-upload") => {
    if (!isR2Configured) {
        throw new Error("Cloudflare R2 is not configured. Please check your environment variables.");
    }
...
export const generatePresignedUrl = async (key, contentType, expiresIn = 3600 // 1 hour default
) => {
    if (!isR2Configured) {
        throw new Error("Cloudflare R2 is not configured. Please check your environment variables.");
    }
...
export const getSignedUrl = async (key, expiresIn = 3600 // 1 hour default
) => {
    if (!isR2Configured) {
        throw new Error("Cloudflare R2 is not configured. Please check your environment variables.");
    }
