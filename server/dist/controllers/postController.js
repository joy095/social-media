...
import { deleteFromR2, uploadToR2, uploadVideoToR2, } from "../utils/r2";
...
class PostController {
                ...
                const uploadPromises = req.files.map(async (file) => {
                    try {
                        ...
                        if (mediaItem.type === "image") {
                            ...
                            try {
                                const uploadResult = await uploadVideoToR2(processedVideo.videoPath);
                                mediaItem.url = uploadResult.secure_url;
                                mediaItem.publicId = uploadResult.public_id;
                                mediaItem.duration = processedVideo.duration;
                                if (processedVideo.thumbnailPath) {
                                    const thumbnailResult = await uploadToR2(processedVideo.thumbnailPath);
                ...
                try {
                    ...
                    mediaFiles.forEach((file) => {
                        try {
                            ...
                            console.error(`Failed to cleanup R2 file ${file.publicId}:`, cleanupError);
                        }
                    });
                    throw uploadError;
                }
    ...
    async getPosts(req, res, next) {
        try {
            ...
            const postsWithInteraction = posts.map((post) => ({
                ...post,
                isLikedByUser: post.likes?.some((like) => like.user?.toString() === req.user.id.toString()) || false,
                userHasViewed: post.views?.some((view) => view.user?.toString() === req.user.id.toString()) || false,
            }));
    ...
    async updatePost(req, res, next) {
        try {
            ...
            const updatedPost = await Post.findByIdAndUpdate(req.params.id, updateData, { new: true, runValidators: true }).populate("author", "fullName username profilePicture role");
            res.status(200).json({
                success: true,
    ...
    async canViewUserProfile(targetUser, currentUserId) {
    ...
    detectBotBehavior(user, viewDuration, screenPercentage) {
