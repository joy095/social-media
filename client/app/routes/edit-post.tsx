...
export function meta({ params }: { params: { id: string } }) {
  return [
    { title: "Edit Post - Social App" },
    { name: "description", content: "Edit your post" },
  ];
}
...
export default function EditPost() {
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const totalImages = formData.images.length + formData.existingImages.length + validFiles.length;
    if (totalImages > 5) {
      toast.error('You can only have up to 5 images per post');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.content.trim() && formData.images.length === 0 && formData.existingImages.length === 0) {
    ...
    try {
      ...
      postData.append('existingImages', JSON.stringify(formData.existingImages));
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-12">
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-6">
          The post you're trying to edit doesn't exist or may have been deleted.
        </p>
        <button
          onClick={() => navigate(-1)}
        ...
        {formData.existingImages.length > 0 && (
          <div className="mb-4">
            ...
            <div className={`grid gap-2 ${
              formData.existingImages.length === 1 ? 'grid-cols-1' :
              formData.existingImages.length === 2 ? 'grid-cols-2' :
              'grid-cols-3'
            }`}>
        ...
        <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              title={formData.images.length + formData.existingImages.length >= 5 ? 'Maximum 5 images allowed' : 'Add images'}
            >
              <ImageIcon size={20} />
            </button>
          ...
          <div className="flex space-x-3">
            ...
            <button
              type="submit"
              disabled={isSubmitting || (!formData.content.trim() && formData.images.length === 0 && formData.existingImages.length === 0)}
